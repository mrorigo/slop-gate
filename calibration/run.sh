...
while IFS='	' read -r name url; do
    ...
    while [ "$attempt" -lt "$retries" ]; do
        skip=$((attempt * 20))
        candidate=$(git -C "$checkout" log --first-parent --skip="$skip" --format='%H' -n 1)
        test -n "$candidate" || break
        window="$repo_output/.history-$attempt.json"
        if (cd "$checkout" && "$binary" history --ref "$candidate" --count 20 --complexity-cutoffs "$cutoffs" --format json > "$window" 2>>"$repo_output/stderr.log") && jq -e 'type == "array" and length == 20' "$window" >/dev/null; then
            cp "$window" "$repo_output/history.json"
    ...
    printf 'selected_ref = "%s"\nretries_used = %s\n' "$selected_ref" "$((attempt + 1))" >> "$output/manifest.lock"
    : > "$repo_output/commits.jsonl"
    if [ -n "$selected_ref" ]; then
    metadata="$repo_output/.metadata.json"
    git -C "$checkout" log --first-parent --format='%H%x09%P%x09%cI' -n 20 "$selected_ref" |
